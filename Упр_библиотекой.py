"""
Задача: Система управления библиотекойУровень сложности:
СреднийПроверяемые концепции: Классы, наследование,
инкапсуляция,полиморфизм, магические методы,
композицияУсловие задачи:Создайте систему управления
библиотекой с использованием ООП.
Система должна работать с книгами, читателями и операциями выдачи/возврата.
Требования:Класс Book:Хранит информацию о книге (название, автор, ISBN, год издания)
Свойства для получения информации о книгеМагический метод str для красивого выводаКнига может быть доступна или выдана
Класс Reader:Хранит информацию о читателе (имя, номер читательского билета)Может брать книги (не более 3 одновременно)
Может возвращать книгиСвойство для получения списка взятых книг

Класс Library:Управляет коллекцией книг и читателями
Добавляет и удаляет книгиРегистрирует читателейВыдает
книги читателям
Принимает книги обратноПоказывает список доступных книг
Показывает список должниковИсключения:
...
ReaderLimitExceededException — читатель достиг лимита книг
ReaderNotFoundException — читатель не найден
Дополнительно:Используйте @property для геттеров/сеттеров где уместно
Добавьте аннотации типовНапишите несколько примеров использования
